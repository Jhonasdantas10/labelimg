...
class MainWindow(QMainWindow, WindowMixin):
    def copy_all_shapes(self):
        """Permite selecionar quais boxes copiar"""
        if not self.canvas.shapes:
            self.statusBar().showMessage('Nenhuma box para copiar')
            return
        
        # Criar diálogo de seleção
        dialog = QDialog(self)
        dialog.setWindowTitle('Selecione as boxes para copiar')
        ...
        checkboxes = []
        original_colors = []  # Guardar as cores originais
        
        for i, shape in enumerate(self.canvas.shapes):
            # Criar checkbox com informações da posição
            checkbox = QCheckBox(f'Box {i+1}: {shape.label} (x={int(shape.points[0].x())}, y={int(shape.points[0].y())})')
            checkbox.setChecked(False)  # Começa desmarcado
            
            # Guardar cor original e mudar cor quando checkbox for marcado
            original_colors.append(shape.line_color)
            def make_callback(s=shape, c=original_colors[-1]):
                def callback(state):
                    if state:
                        s.line_color = QColor(0, 0, 255)  # Azul
                    else:
                        s.line_color = c
                    self.canvas.update()
                return callback
        ...
        def toggle_all(state):
            for checkbox in checkboxes:
                ...
                # Atualiza as cores das shapes
                if state == Qt.Checked:
                    self.canvas.shapes[checkboxes.index(checkbox)].line_color = QColor(0, 0, 255)
                else:
                    self.canvas.shapes[checkboxes.index(checkbox)].line_color = original_colors[checkboxes.index(checkbox)]
            self.canvas.update()
        
        select_all_checkbox.stateChanged.connect(toggle_all)
        
        # Configurar widget de scroll
        scroll_content.setLayout(scroll_layout)
        scroll.setWidget(scroll_content)
        
        # Definir tamanho máximo do scroll
        ...
        main_layout.addWidget(select_all_checkbox)  # Adiciona checkbox "Selecionar Todos" no topo
        ...
        try:
            if dialog.exec_():
                # Copiar apenas as boxes selecionadas
                self.copied_shapes = []
                for checkbox, shape in zip(checkboxes, self.canvas.shapes):
                    if checkbox.isChecked():
                        copied_shape = Shape(shape.label)
                        copied_shape.points = shape.points[:]
                        copied_shape.fill_color = shape.fill_color
                        copied_shape.line_color = shape.line_color
                        copied_shape.difficult = shape.difficult
                        self.copied_shapes.append(copied_shape)
            
                self.statusBar().showMessage(f'Copiadas {len(self.copied_shapes)} boxes')
            else:
                # Se cancelou, inicializa copied_shapes como lista vazia
                self.copied_shapes = []
                self.statusBar().showMessage('Operação cancelada')
        finally:
            # Restaurar cores originais
            for shape, color in zip(self.canvas.shapes, original_colors):
                shape.line_color = color
            self.canvas.update()

    def paste_all_shapes(self):
        """Cola todas as boxes copiadas na imagem atual"""
        if not hasattr(self, 'copied_shapes') or not self.copied_shapes:
            self.statusBar().showMessage('Nenhuma box copiada')
            return
        
        # Colar as shapes na imagem atual
        for shape in self.copied_shapes:
            # Verificar sobreposição
            has_overlap = False
            for existing_shape in self.canvas.shapes:
                if self._shapes_overlap(shape, existing_shape):
                    has_overlap = True
                    break
            ...
            if has_overlap:
                ...
                msg.setText(f"Coordenada existente na imagem! A colagem será ignorada.")
                msg.setWindowTitle("Aviso")
                msg.exec_()
                return
            
            new_shape = Shape(shape.label)
    ...
    def paste_shapes_interval(self):
        """Cola as boxes em um intervalo de imagens"""
        if not hasattr(self, 'copied_shapes') or not self.copied_shapes:
            self.statusBar().showMessage('Nenhuma box copiada')
            return
        
        # Criar diálogo para selecionar intervalo
        dialog = QDialog(self)
        dialog.setWindowTitle('Selecione o intervalo de imagens')
        
        # Layout principal
        layout = QVBoxLayout()
        
        # Criar widgets para entrada do intervalo
        form_layout = QFormLayout()
        
        # Spinboxes para início e fim
        start_spinbox = QSpinBox()
        end_spinbox = QSpinBox()
        
        # Configurar limites baseados na quantidade de imagens
        start_spinbox.setMinimum(1)
        start_spinbox.setMaximum(self.img_count)
        start_spinbox.setValue(self.cur_img_idx + 1)  # Valor atual
        
        end_spinbox.setMinimum(1)
        end_spinbox.setMaximum(self.img_count)
        end_value = min(self.cur_img_idx + 9, self.img_count)
        end_spinbox.setValue(end_value)
        
        # Adicionar campos ao form layout
        form_layout.addRow('Imagem inicial:', start_spinbox)
        form_layout.addRow('Imagem final:', end_spinbox)
        ...
        if dialog.exec_():
            start_idx = start_spinbox.value() - 1  # Converter para índice base 0
            end_idx = end_spinbox.value() - 1
            
            # Validar intervalo
            if start_idx > end_idx:
                msg = QMessageBox()
                msg.setIcon(QMessageBox.Warning)
                msg.setText("Intervalo inválido!")
            ...
            skipped_images = []  # Lista para guardar imagens ignoradas
            
            try:
                # Iterar sobre o intervalo selecionado
                for idx in range(start_idx, end_idx + 1):
                    ...
                    # Verificar sobreposição para a imagem atual
                    has_overlap = False
                    for new_shape in self.copied_shapes:
                        for existing_shape in self.canvas.shapes:
                            if self._shapes_overlap(new_shape, existing_shape):
                                has_overlap = True
                                skipped_images.append(idx + 1)  # Adiciona à lista de ignorados
                                break
                        if has_overlap:
                            break
                    ...
                    if has_overlap:
                        msg = QMessageBox()
                        msg.setIcon(QMessageBox.Warning)
                        msg.setText(f"Coordenada existente na imagem {idx + 1}! Esta imagem será ignorada.")
                        msg.setWindowTitle("Aviso")
                        msg.exec_()
                        continue  # Pula para a próxima imagem
                    
                    # Colar as shapes se não houver sobreposição
                    for shape in self.copied_shapes:
                        new_shape = Shape(shape.label)
                        new_shape.points = shape.points[:]
                        new_shape.fill_color = shape.fill_color
                ...
                # Mensagem final com informações sobre imagens ignoradas
                if skipped_images:
                    status_msg = f'Boxes coladas no intervalo {start_idx + 1} até {end_idx + 1}. Imagens ignoradas: {", ".join(map(str, skipped_images))}'
                else:
                    status_msg = f'Boxes coladas em todas as imagens do intervalo {start_idx + 1} até {end_idx + 1}'
                self.statusBar().showMessage(status_msg)
                
            except Exception as e:
                self.statusBar().showMessage(f'Erro ao colar boxes: {str(e)}')
                print(f"Erro: {str(e)}")  # Para debug
                
            finally:
                # Voltar para a imagem original
                self.cur_img_idx = current_idx
                self.load_file(self.m_img_list[current_idx])

    def _shapes_overlap(self, shape1, shape2):
        """Verifica se duas shapes estão na mesma posição"""
        # Pega os pontos das duas shapes
        points1 = shape1.points
        points2 = shape2.points
        
        # Verifica se os pontos são idênticos
    ...
    def __init__(self, default_filename=None, default_prefdef_class_file=None, default_save_dir=None):
        ...
        # Definir caminhos dos ícones
        icon_dir = os.path.join(os.path.dirname(__file__), 'resources', 'icons')
        copy_all = action(get_str('copyAll'), self.copy_all_shapes,
                    'C', None, get_str('copyAll'), enabled=True)
        copy_all.setIcon(new_icon('copy-all'))
        
        paste_all = action(get_str('pasteAll'), self.paste_all_shapes,
                    'R', None, get_str('pasteAll'), enabled=True)
        paste_all.setIcon(new_icon('paste-all'))
        
        paste_interval = action(get_str('pasteInterval'), self.paste_shapes_interval,
                        'V', None, get_str('pasteInterval'), enabled=True)
        paste_interval.setIcon(new_icon('paste-interval'))

        # Versão final com o ícone delete
        delete_selected = action(get_str('deleteSelected'), self.delete_selected_shapes,
                        'Ctrl+Shift+D', None, get_str('deleteSelected'), enabled=True)
        delete_selected.setIcon(new_icon('delete'))
        ...
        delete_image = action(get_str('deleteImg'), self.delete_image, 'F4', 'close', get_str('deleteImgDetail'))
        ...
        delete = action(get_str('delBox'), self.delete_selected_shape,
                        'e', 'delete', get_str('delBoxDetail'), enabled=False)
        ...
        self.actions = Struct(save=save, save_format=save_format, saveAs=save_as, open=open, close=close, resetAll=reset_all, deleteImg=delete_image,
                              ...
                              editMenu=(
                                  ...
                                  delete_selected,
                                  None,
                                  color1, self.draw_squares_option
                              ),
        ...
        # Adicionar ao toolbar no início da lista de ações
        self.actions.beginner = (
            open, open_dir, change_save_dir, 
            copy_all, paste_all, paste_interval, delete_selected,  # Adicionado delete_selected aqui
            open_next_image, open_prev_image, verify, save, save_format, None,
            create, copy, delete, None,
            zoom_in, zoom, zoom_out, fit_window, fit_width, None,
            light_brighten, light, light_darken, light_org
        )

        # Criar e configurar a toolbar
    ...
    def paint_canvas(self):
        ...
        self.canvas.scale = 0.007 * self.zoom_widget.value()
    ...
    def adjust_scale(self, initial=False):
        ...
        self.zoom_widget.setValue(int(130 * value))
    ...
    def delete_selected_shapes(self):
        """Permite selecionar quais boxes apagar em um intervalo de imagens"""
        if not self.canvas.shapes:
            self.statusBar().showMessage('Nenhuma box para apagar')
            return
        
        # Criar diálogo de seleção
        dialog = QDialog(self)
        dialog.setWindowTitle('Selecione as boxes para apagar')
        
        # Criar layout principal
        main_layout = QVBoxLayout()
        
        # Criar widget para seleção do intervalo de imagens
        ...
        range_layout.addWidget(QLabel('Imagem inicial:'))
        range_layout.addWidget(start_spinbox)
        range_layout.addWidget(QLabel('Imagem final:'))
        ...
        for i, shape in enumerate(self.canvas.shapes):
            ...
            def make_callback(s=shape, c=original_colors[-1]):
                def callback(state):
                    if state:
                        s.line_color = QColor(255, 0, 0)  # Vermelho para indicar que será deletado
                    else:
                        s.line_color = c
                    self.canvas.update()
                return callback
        ...
        def toggle_all(state):
            for checkbox in checkboxes:
                ...
                # Atualiza as cores das shapes
                if state == Qt.Checked:
                    self.canvas.shapes[checkboxes.index(checkbox)].line_color = QColor(255, 0, 0)
                else:
                    self.canvas.shapes[checkboxes.index(checkbox)].line_color = original_colors[checkboxes.index(checkbox)]
            self.canvas.update()
        ...
        try:
            if dialog.exec_():
                ...
                try:
                    # Selecionar shapes para apagar
                    selected_shapes = []
                    for checkbox, shape in zip(checkboxes, self.canvas.shapes):
                        if checkbox.isChecked():
                            selected_shapes.append((shape.label, shape.points))
                    ...
                    for idx in range(start_idx, end_idx + 1):
                        ...
                        # Encontrar e apagar shapes correspondentes
                        shapes_to_delete = []
                        for shape in self.canvas.shapes:
                            for sel_label, sel_points in selected_shapes:
                                if shape.label == sel_label and all(
                                    abs(p.x() - sp.x()) < 1 and abs(p.y() - sp.y()) < 1 
                                    for p, sp in zip(shape.points, sel_points)):
                                    shapes_to_delete.append(shape)
                                    break
                        
                        # Apagar as shapes encontradas
                    ...
                    self.statusBar().showMessage(f'Apagadas {deleted_count} boxes no intervalo de imagens {start_idx + 1} até {end_idx + 1}')
